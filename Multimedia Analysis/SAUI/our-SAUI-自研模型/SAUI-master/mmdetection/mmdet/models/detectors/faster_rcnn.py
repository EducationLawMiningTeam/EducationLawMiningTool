...
class FasterRCNN(TwoStageDetector):
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            rpn_outs = self.rpn_head(x)     # cls_scores, pred_bboxes
    ...
    def feats_extract(self,
        ...
        if self.bbox_head.num_shared_fcs > 0:
            ...
            b_f_copy = bbox_feats
            b_f_tem = bbox_feats.new_zeros((bbox_feats.shape[0], bbox_feats.shape[1], self.bbox_head.fc_out_channels))
            for i in range(bbox_feats.shape[1]):
                bbox_feats = b_f_copy[:, i, :, :]
