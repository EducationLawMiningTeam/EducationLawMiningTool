...
class BBoxTestMixin(object):
    ...
    def test_zsd(self, rcnn_test_cfg, cls_score, bbox_pred):
        ...
        seen_softmax = F.softmax(seen_score,dim=1)
        seen_argmax_ind2 = seen_softmax.argmax(1)
        seen_argmax_ind1 = list(range(seen_argmax_ind2.shape[0]))
        seen_softmax_max = seen_softmax[seen_argmax_ind1, seen_argmax_ind2]
        unseen=unseen_score.argmax(1)
        unseen_softmax = F.softmax(unseen_score,dim=1)
        unseen_argmax_ind2 = unseen_softmax.argmax(1)
        unseen_argmax_ind1 = list(range(unseen_argmax_ind2.shape[0]))
        unseen_softmax_max = unseen_softmax[unseen_argmax_ind1,unseen_argmax_ind2]
        ...
        high_in_seen_ind = np.where(seen_softmax_max.data.cpu().numpy()>seen_thres)[0]
        low_in_unseen_ind = np.where(unseen_softmax_max.data.cpu().numpy()<unseen_thres)
        high_low_ind = np.intersect1d(low_in_unseen_ind, high_in_seen_ind)
        cls_score[high_low_ind, 0] = 1.0e3
    ...
    def test_gzsd(self, rcnn_test_cfg, cls_score, bbox_pred):
        ...
        elif rcnn_test_cfg.dataset_name=='voc' or rcnn_test_cfg.dataset_name == 'vd':
