...
class ConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            self.fc_cls = nn.Linear(self.cls_last_dim*4, self.num_classes)
        if self.with_reg:
            out_dim_reg = (4 if self.reg_class_agnostic else 4 *
                           self.num_classes)
            self.fc_reg = nn.Linear(self.reg_last_dim*4, out_dim_reg)
    ...
    def forward(self, x, return_cls_feats=False):
        '''
            x = tensor[512*batch_size, 4, 256, 7, 7]
        '''
        x_copy=x
        x_tem=x.new_zeros((4, x.shape[0], 1024))
        for i in range(4):
            x=x_copy[:, i, :, :]
        # shared part
        ...
        x=torch.cat((x_tem[0],x_tem[1],x_tem[2],x_tem[3]), dim=1)
