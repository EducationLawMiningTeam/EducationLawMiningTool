...
VD_ALL_CLASSES = np.array([
    'airplane', 'baseballfield', 'bridge', 'chimney', 'dam', 'Expressway-Service-area',
...
def get_unseen_coco_cat_ids(split='65_15'):
    ...
    # print(f'unseen ids for coco : {ids}')
    return ids
...
def get_unseen_vd_ids():
    ids = np.where(np.isin(VD_ALL_CLASSES, VD_UNSEEN_CLASSES))[0] + 1
    return ids
...
def get_seen_vd_ids():
    seen_classes = np.setdiff1d(VD_ALL_CLASSES, VD_UNSEEN_CLASSES)
    ids = np.where(np.isin(VD_ALL_CLASSES, seen_classes))[0] + 1
    return ids
