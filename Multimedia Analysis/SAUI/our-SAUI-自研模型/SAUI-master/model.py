...
class MLP_CG(nn.Module):
    def __init__(self, opt):
        super(MLP_CG, self).__init__()
        self.fc1 = nn.Linear(opt.attSize + opt.nz + opt.resSize, opt.ngh)
        self.fc2 = nn.Linear(opt.ngh, opt.resSize)
        self.lrelu = nn.LeakyReLU(0.2, True)
        #self.prelu = nn.PReLU()
        self.relu = nn.ReLU(True)
    ...
    def forward(self, noise, att, res):
        # uneven batch size
        noise = noise[:att.shape[0], :]
        h = torch.cat((noise, att, res), 1)
        h = self.lrelu(self.fc1(h))
        h = self.relu(self.fc2(h))
        return h
