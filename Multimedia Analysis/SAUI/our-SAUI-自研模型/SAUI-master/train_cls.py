...
class TrainCls():
    ...
    def init_model(self):
        self.classifier = ClsModelTrain(num_classes=len(self.classes_to_train)) # MLP:4096(feature_size)->num_classes
    ...
    def load_checkpoint(self, load_path, resume_train):
        if resume_train == 1:
            checkpoint = torch.load(load_path+"classifier_best.pth")
            self.classifier.load_state_dict(checkpoint['state_dict'])
        elif resume_train ==2:
            checkpoint = torch.load(load_path+"classifier_latest.pth")
            self.classifier.load_state_dict(checkpoint['state_dict'])
    ...
    def trainEpochs(self):
        """
        Train syn_cls using syn_features and syn_labels
        """
        for epoch in range(self.opt.nepoch_cls):
            ...
            for ite, (in_feat, in_label)  in enumerate(self.dataloader):
                in_feat = in_feat.type(torch.float)
                in_feat = torch.cat((in_feat[:,0], in_feat[:,1], in_feat[:,2], in_feat[:, 3]), dim=1)
            ...
            if self.best_acc <= self.valacc:
                torch.save({'state_dict': self.classifier.state_dict(),'gan_epoch': self.gan_epoch, 'cls_epoch': epoch, 'acc': self.valacc}, f"{self.opt.outname}/classifier_best.pth")
            ...
            if self.isBestIter:
                ...
                torch.save({'state_dict': self.classifier.state_dict(),'gan_epoch': self.gan_epoch, 'cls_epoch': epoch, 'acc': self.valacc}, f"{self.opt.outname}/classifier_best_latest.pth")
            print(f"saved interminate model accuracy : {self.valacc:0.6f}")
