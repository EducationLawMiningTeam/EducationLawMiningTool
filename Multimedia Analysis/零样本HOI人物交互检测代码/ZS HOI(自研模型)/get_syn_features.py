# 由final_train_cls改变一点点

# 最新的更新分类器参数的文件，这是一个单独的文件，所有训练完成后进行
...
from train_cls import TrainHoiCls

from hoi_train_gan import TrainGAN
...
features, labels, actions, objs = seenDataset.epochData(include_bg=False)

# synthesize features
for i in range(20):
    trainFGGAN.netG = torch.load('results_{}_{}/G_64_{}_{}.pth'.format( opt.zero_shot_type, opt.use_type, i*5 + 194, opt.zero_shot_type)).cuda()
    

    syn_feature, syn_action, syn_obj = trainFGGAN.generate_syn_feature(unseen_att_labels, unseen_attributes, unseenLabels_obj, 1000 , no_grad=True)  # 这个1000之后要改回去


    labels = []
    syn_feature = np.array(syn_feature)
    for lable in unseen_att_labels:
        for j in range(1000):
            labels.append(lable)

    syn_label = np.array(labels).astype(int)

    np.save('show_data/{}_syn_feature_{}_{}.npy'.format(opt.zero_shot_type, opt.use_type, i*5 + 194), syn_feature)
    print('show_data/{}_syn_feature_{}_{}.npy'.format(opt.zero_shot_type, opt.use_type, i*5 + 194))
    np.save('show_data/{}_syn_label_{}_{}.npy'.format(opt.zero_shot_type, opt.use_type, i*5 + 194), syn_label)
