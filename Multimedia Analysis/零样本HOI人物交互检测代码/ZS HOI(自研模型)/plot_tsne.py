...
labels_to_plot = np.array( [18, 245, 577] ) 
...
def tsne_plot_feats(f_feat, f_labels, path_save):
    ...
    idx = np.random.permutation(idx)[0:400]   # 原来是2000
...
def plot_unseen(epochs=20):
    real_f, real_l = tsne_plot_feats('show_data/{}_feats_all.npy'.format(opt.zero_shot_type), 'show_data/{}_labels_all.npy'.format(opt.zero_shot_type), 'show_data/{}_real_tsne.png'.format(opt.zero_shot_type))
    print(f"len of real feats: {len(real_f)}")
    for epoch in range(0, epochs):
        f_feat = 'show_data/{}_syn_feature_{}_{}.npy'.format(opt.zero_shot_type, opt.use_type, epoch*5 + 194)
        f_labels = 'show_data/{}_syn_label_{}_{}.npy'.format(opt.zero_shot_type, opt.use_type, epoch*5 + 194)
        path_save = 'show_data/{}_unseen_{}_{}.png'.format(opt.zero_shot_type, opt.use_type, epoch*5 + 194)
        ...
        plt.legend()
        fig.savefig('show_data/u_{}_both_{}_{}.png'.format(opt.zero_shot_type, opt.use_type, epoch*5 + 194))
