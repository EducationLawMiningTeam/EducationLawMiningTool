...
class FeaturesCls(Dataset):
    def __init__(self, opt, features=None, actions=None, objs=None): # val=False, split='seen', classes_to_train=None):
        self.root = f"data_{opt.zero_shot_type}"
        self.opt = opt
    ...
    def __getitem__(self, idx):
        ...
        return batch_feature, batch_action, batch_obj
...
class FeaturesGAN():
    def __init__(self, opt):
        ...
        self.all_features = torch.from_numpy(self.all_features).cuda()
        self.all_labels = np.load(f"{self.root}/{self.opt.trainsplit}_labels_label.npy")
        self.all_labels = torch.from_numpy(self.all_labels).cuda()
        self.all_actions = np.load(f"{self.root}/{self.opt.trainsplit}_actions.npy")
        self.all_actions= torch.from_numpy(self.all_actions).cuda()
        self.all_objs= np.load(f"{self.root}/{self.opt.trainsplit}_objs.npy")
        self.all_objs= torch.from_numpy(self.all_objs).cuda()
        mean_path = f"{self.root}/{self.opt.trainsplit}_mean.npy"
        print(f'loaded data from {self.opt.trainsplit}')
        self.pos_inds = [i for i in range(len(self.all_labels))]
        self.neg_inds = [-1]

        unique_labels = np.array([1,2,3])
        self.num_bg_to_take = 00000000
    ...
    def epochData(self, include_bg=False):
        ...
        actions = self.all_actions[inds]
        objs = self.all_objs[inds]
        return features, labels, actions, objs
