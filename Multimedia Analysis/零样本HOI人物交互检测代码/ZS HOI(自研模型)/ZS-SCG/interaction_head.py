...
class GraphHead(Module):
    ...
    def forward(self,
        ...
        -----------
            features: OrderedDict
                Feature maps returned by FPN
            box_features: Tensor
                (N, C, P, P) Pooled box features   # ?? 这里的(N, C, P, P)
            image_shapes: List[Tuple[int, int]]
                Image shapes, heights followed by widths
        ...
        # torch.save(global_features, 'global_features_example')
    ...
    # def my_forward(self,    # 这个是根据第一种特征提取方法，提出的特征合并方法
    #     global_features: Tensor,
    #     human_features: Tensor,
    #     obj_features: Tensor,
    #     box_pair_spatial: Tensor,
    #     obj_labels: List[Tensor],
    # ):
    #     """
    #     更改forward来实现合并 人 物 空间 特征
    #     """

    #     box_features = torch.cat((human_features.unsqueeze(0), obj_features.unsqueeze(0)), dim=0)
    #     box_labels = [torch.cat((obj_labels.unsqueeze(0), torch.tensor(49).unsqueeze(0).cuda() ))]
    #     box_scores = [torch.ones(2)]
    
    #     counter = 0
    #     all_boxes_h = []; all_boxes_o = []; all_object_class = []
    #     all_labels = []; all_prior = []
    #     all_box_pair_features = []
    #     for b_idx, (labels, scores) in enumerate(zip(box_labels, box_scores)):  # for是针对batch中的数据,但我们batch一直是1
    #         n = 2  # 当前图片的box数
    #         device = box_features.device
    ...
    #         node_encodings = box_features[counter: counter+n]
    #         # Duplicate human nodes
    #         h_node_encodings = node_encodings[:n_h]  # 前面n_h个都是human
    #         # Get the pairwise index between every human and object instance
    #         x, y = torch.meshgrid(
    #             torch.arange(n_h, device=device),
    #             torch.arange(n, device=device)
    ...
    #     return all_box_pair_features


    def forward_for_extract_features(self,
        features: OrderedDict, image_shapes: List[Tuple[int, int]],
        box_features: Tensor, box_coords: List[Tensor],
        box_labels: List[Tensor], box_scores: List[Tensor],
        targets: Optional[List[dict]] = None
        ...
        for b_idx, (coords, labels, scores) in enumerate(zip(box_coords, box_labels, box_scores)):
            ...
            h_node_encodings = node_encodings[:n_h]
            o_node_encodings = node_encodings[n_h:]  ###新加
            # Get the pairwise index between every human and object instance
            x, y = torch.meshgrid(
                torch.arange(n_h, device=device),
                torch.arange(n, device=device)
            ...
            # of the humans included amongst object nodes
            x = x.flatten(); y = y.flatten()
            x = torch.tensor([0], dtype=torch.int64).cuda(); y = torch.tensor([1], dtype=torch.int64).cuda()
            ### n
            n = 1
            # Compute spatial features
            box_pair_spatial = compute_spatial_encodings(
            ...
            all_box_pair_features.append(torch.cat([
                self.attention_head(
                    ...
                    box_pair_spatial_reshaped[x_keep, x_keep]
                ), self.attention_head_g(
                    global_features[b_idx, None],
                    box_pair_spatial_reshaped[x_keep, x_keep])
            ], dim=1))
    ...
    def forward_for_compose_features(self,    
        global_features: Tensor,
        human_features: Tensor,
        obj_features: Tensor,
        box_pair_spatial: Tensor,
        ...
        for b_idx, (labels, scores) in enumerate(zip(box_labels, box_scores)):  # for是针对batch中的数据,但我们batch一直是1
            ...
            # Duplicate human nodes
            h_node_encodings = node_encodings[:n_h]  # 前面n_h个都是human
            o_node_encodings = node_encodings[n_h:] 
            # Get the pairwise index between every human and object instance
            x, y = torch.meshgrid(
                torch.arange(n_h, device=device),
                torch.arange(n, device=device)
