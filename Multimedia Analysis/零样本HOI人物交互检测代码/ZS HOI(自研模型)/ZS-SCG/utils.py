    ...
    def __init__(self,
        ...
        if name == 'hicodet':
            ...
            if partition == 'train2015':
                self.dataset = HICODet(
                    root=os.path.join(data_root, 'hico_20160224_det/images', partition),
                    anno_file=os.path.join(data_root, 'instances_{}_{}.json'.format(partition, zero_shot_type)),
                    target_transform=pocket.ops.ToTensor(input_format='dict')
                )
                self.human_idx = 49
...
def test(net, test_loader):
    ...
    meter = DetectionAPMeter(600, nproc=1,num_gt=testset.anno_interaction, algorithm='11P')
    net.eval()
    for batch in test_loader:
        inputs = pocket.ops.relocate_to_cuda(batch[:-1])
        if inputs[1][0]['boxes'].numel() == 0:
            continue
        with torch.no_grad():
            output = net(*inputs)
        if output is None:
        ...
        for batch_i in range(len(output)):
            # Handle output batch_i
            output_i = pocket.ops.relocate_to_cpu(output[batch_i])
            target_i = batch[-1][batch_i]
            # Format detections
            box_idx = output_i['index']
            boxes_h = output_i['boxes_h'][box_idx]
            boxes_o = output_i['boxes_o'][box_idx]
            objects = output_i['object'][box_idx]
            scores = output_i['scores']
            verbs = output_i['prediction']
            interactions = torch.tensor([testset.object_n_verb_to_interaction[o][v] for o, v in zip(objects, verbs)])
            # Associate detected pairs with ground truth pairs
            labels = torch.zeros_like(scores)
            unique_hoi = interactions.unique()
            for hoi_idx in unique_hoi:
                gt_idx = torch.nonzero(target_i['hoi'] == hoi_idx).squeeze(1)
                det_idx = torch.nonzero(interactions == hoi_idx).squeeze(1)
                if len(gt_idx):
                    labels[det_idx] = associate(
                        (target_i['boxes_h'][gt_idx].view(-1, 4), target_i['boxes_o'][gt_idx].view(-1, 4)),
