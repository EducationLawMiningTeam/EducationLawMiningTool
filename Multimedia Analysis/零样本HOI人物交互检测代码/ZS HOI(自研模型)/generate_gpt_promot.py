...
client = OpenAI(
    ...
    api_key="sk-CibiOpi5sZyi9G8gne0wGCLSI6lGyercy889vnVDz6avDPqR",
...
text = """(4, 4): 'a photo of a person boarding an airplane',
...
for sentence in sentences: 
    # print("'{}',".format(sentence[11:]))



    messages = [{'role': 'user','content': 'Please retrieve the knowledge for {}, limited to 50 words, mainly focusing on action interaction, limited to 50 words. Your answer can refer to these two templates(not asking you to answer two points).\
                1.Riding a bicycle involves balance, coordination, and physical exertion. The rider mounts the bike, propels forward\
                by pushing pedals with their feet. Steering is achieved by turning handlebars. Brakes slow or stop the bike. Helmets\
                are worn for safety. 2.Throwing a frisbee involves grasping the disc, typically with a forehand grip, then swinging the\
                arm and releasing the frisbee at the right moment for it to glide through the air. Direction and distance depend on the angle\
                and speed of the throw.'.format(sentence[11:])}]
    # 非流式调用
    # gpt_35_api(messages)
    # 流式调用
    promot.append(gpt_35_api(messages))

import torch
torch.save(promot, 'hoi_promot')
print(promot)
