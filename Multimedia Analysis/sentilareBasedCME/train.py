...
def prep_for_training(args, num_train_optimization_steps: int):
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                 p for n, p in param_optimizer if not any(nd in n for nd in no_decay)  and not any(nd in n for nd in CME_params)
            ],
...
def train(
    ...
    for epoch_i in range(int(args.n_epochs)):
        ...
        print(
            "epoch-{} : train_loss:{:.4f}, train_acc:{:.4f} / valid_loss:{:.4f}, valid_acc:{:.4f} / test_loss:{:.4f}, test_acc:{:.4f}".format(
                epoch_i + 1, train_loss, train_acc, valid_loss, valid_acc, test_loss, test_acc
            )
        )
