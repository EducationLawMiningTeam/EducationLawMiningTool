...
class Net(nn.Module):
    ...
    def __init__(self, exer_n, student_n, knowledge_n, stu_cnt, exer_cnt, latent_dim, disc_range=1.0):
        self.knowledge_dim = knowledge_n
        self.exer_n = exer_n
        self.stu_n = student_n
        self.stu_dim = self.knowledge_dim
        self.latent_dim = latent_dim if latent_dim > 0 else knowledge_n

        super(Net, self).__init__()

        self.stat_mean = nn.Embedding(student_n, self.latent_dim)
        self.stat_std_eta = nn.Embedding(student_n, self.latent_dim)
        self.diff_mean = nn.Embedding(exer_n, 1)
        self.diff_std_eta = nn.Embedding(exer_n, 1)
        self.disc_mean = nn.Embedding(exer_n, self.latent_dim)
        self.disc_std_eta = nn.Embedding(exer_n, self.latent_dim)
        ...
        nn.init.kaiming_normal_(self.stat_mean.weight)
        nn.init.kaiming_normal_(self.stat_std_eta.weight)
        nn.init.kaiming_normal_(self.diff_mean.weight)
        nn.init.kaiming_normal_(self.disc_std_eta.weight)

    def get_distribution(self, stu_id, exer_id):
        stat_mean = self.stat_mean(stu_id)
        stat_std_model = F.softplus(self.stat_std_eta(stu_id))
        stat_std_data = F.softplus(self.lambda_1_eta_stu) * torch.exp(
            - F.softplus(self.lambda_2_eta_stu) * self.stu_cnt[stu_id].view(-1, 1))
        stat_std = combine_std_(stat_std_data, stat_std_model)
        k_diff_mean = self.diff_mean(exer_id)
        e_disc_mean = self.disc_mean(exer_id)
        diff_std_model = F.softplus(self.diff_std_eta(exer_id))
        disc_std_model = F.softplus(self.disc_std_eta(exer_id))
        e_std_data = F.softplus(self.lambda_1_eta_exer) * torch.exp(
            - F.softplus(self.lambda_2_eta_exer) * self.exer_cnt[exer_id].view(-1, 1))
    ...
    def forward(self, stu_id, exer_id, y, device, sample=True, sample_n=1):
        ...
        if sample:
            stu_stat = self.reparameterize_gaussian(stat_mean, stat_std, sample_n)  # (sample_n, batch_size, 1)
            k_difficulty = self.reparameterize_gaussian(k_diff_mean, k_diff_std, sample_n)  # (sample_n, batch_size, 1)
            e_discrimination = self.reparameterize_gaussian(e_disc_mean, e_disc_std, sample_n)  # (sample_n, batch_size, 1)
        else:
            stu_stat = stat_mean
        ...
        # prednet
        output = torch.sigmoid((e_discrimination * stu_stat).sum(dim=-1, keepdim=True) - k_difficulty)
        ...
        return output.view(sample_n, -1), kl
    ...
    def pred_interval(self, stu_id, exer_id, d, device, sample_n=30):
        with torch.no_grad():
            ...
            if self.disc_range <= 0:
                ...
                assert self.disc_range > 0
                e_discrimination = self.disc_range * torch.sigmoid(e_discrimination)
            output_up = torch.sigmoid((e_discrimination * stat_up).sum(dim=-1, keepdim=True) - k_difficulty)
            output_down = torch.sigmoid((e_discrimination * stat_down).sum(dim=-1, keepdim=True) - k_difficulty)
        return output_up.view(sample_n, -1), output_down.view(sample_n, -1)
...
class UNCDM(CDM):
    def __init__(self, **kwargs):
        ...
        self.net = Net(kwargs['exer_n'], kwargs['student_n'], kwargs['knowledge_n'], kwargs['stu_cnt'],
                       kwargs['exer_cnt'], kwargs['latent_dim'], kwargs['disc_range'])
    ...
    def train(self, train_set, valid_set, test_set, lr=0.002, device='cpu', epoch_n=15, sample_n=1, diag_klw=0.):
        ...
        :return:
        '''
        logging.info("training... (lr={}, sample_n={}, diag_klw={}, a-range={}".format(lr, sample_n, diag_klw, self.net.disc_range))
    ...
    def eval_cwc(self, test_data, d, mu, eta, device="cpu", sample_n=50):
    ...
    def analysis_std_model1(self, data_set, device="cpu"):
        ...
        pred_differ = torch.zeros(self.net.stu_n, self.net.latent_dim).to(device)
        pred_cnt = torch.zeros(self.net.stu_n, self.net.latent_dim).to(device)
        with torch.no_grad():
        ...
        return corr_sum, corr_avg

    def get_pars(self):
        self.net = self.net.cpu()
        with torch.no_grad():
            stu_mean = self.net.stat_mean.weight.data
            stu_std_model = F.softplus(self.net.stat_std_eta.weight.data)
            stu_std_data = F.softplus(self.net.lambda_1_eta_stu) * torch.exp(
                - F.softplus(self.net.lambda_2_eta_stu) * self.net.stu_cnt.view(-1, 1).cpu())
            # stu_std_data = 1 * torch.exp(- 1 * self.net.stu_cnt)
            k_diff_mean = self.net.diff_mean.weight.data
            k_diff_std_model = F.softplus(self.net.diff_std_eta.weight.data)
            e_std_data = F.softplus(self.net.lambda_1_eta_exer) * torch.exp(
                - F.softplus(self.net.lambda_2_eta_exer) * self.net.exer_cnt.view(-1, 1).cpu())
            # e_std_data = 1 * torch.exp(- 1 * self.net.exer_cnt)
            e_disc_mean = self.net.disc_mean.weight.data
            e_disc_std_model = F.softplus(self.net.disc_std_eta.weight.data)
...
def calculate_picp(data_set, model: UNCDM, path_prefix, epochs, d, sample_n, device):
    logging.info('calculate_picp ... (d={}, sample_n={})'.format(d, sample_n))
    for i in epochs:
        model.load('{}{}'.format(path_prefix, i))
        picp = model.eval_picp(test_data=data_set, d=d, sample_n=sample_n, device=device)
        logging.info('epoch{}: PICP={}'.format(i, picp))
        print('epoch{}: PICP={}'.format(i, picp))
...
def corelation_std_data(data_name, path_prefix, model: UNCDM, epoch=None):
    ...
    for epoch_i in epoch_range:
        ...
        with torch.no_grad():
            ...
            diff_std = combine_std_(e_std_data, k_diff_std_model).numpy().reshape(-1)
            disc_std = combine_std_(e_std_data, e_disc_std_model).numpy()
            stu_cnt = model.net.stu_cnt.cpu().view(-1, 1).repeat(1, model.net.latent_dim).view(-1)
            sp_stu = pd.Series(stu_std.reshape(-1)).corr(pd.Series(stu_cnt.numpy()), method='spearman')
            exer_cnt = model.net.exer_cnt.cpu().numpy().reshape(-1)
            sp_diff = pd.Series(diff_std).corr(pd.Series(exer_cnt), method='spearman')
            exer_cnt = model.net.exer_cnt.cpu().view(-1, 1).repeat(1, model.net.latent_dim).view(-1)
            sp_disc = pd.Series(disc_std.reshape(-1)).corr(pd.Series(exer_cnt), method='spearman')
...
def analyse_std_model1(path_prefix, model: UNCDM, epoch=None, device='cpu'):
    ...
    for epoch_i in epoch_range:
        ...
        model.load(model_path)
        with torch.no_grad():
            corr = model.analysis_std_model1(train_set, device)
            print(f'epoch {epoch_i}, corr_sum={corr[0]}')
            logging.info(f'epoch {epoch_i}, corr_sum={corr[0]}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--combine', type=str, default='x')    # x 是和论文一致的
    ...
    if not os.path.isdir(dst_folder):
        os.makedirs(dst_folder)
    logging.basicConfig(filename='{}/{}-s{}dkl{}cb{}ar{}ld{}.log'.format(dst_folder, model_name, args.sample_n, args.diag_klw, args.combine, args.a_range, args.latent_dim),
                        level=logging.INFO, format='%(asctime)s %(message)s')

    stu_n, exer_n, knowledge_n, train_set, valid_set, test_set, stu_cnt, exer_cnt = load_data_info(args.data)
    uncd = UNCDM(exer_n=exer_n, student_n=stu_n, knowledge_n=knowledge_n, stu_cnt=stu_cnt, exer_cnt=exer_cnt, latent_dim=args.latent_dim, disc_range=args.a_range)
    model_prefix = 'result/{}/{}/{}-s{}dkl{}cb{}ar{}ld{}.snapshot'.format(data_name, model_name, model_name, args.sample_n,
                                                                  args.diag_klw, args.combine, args.a_range, args.latent_dim)
