...
from sklearn.metrics import roc_auc_score, accuracy_score, \
    mean_squared_error, recall_score, precision_score, f1_score
...
class DINA:
    ...
    def train(self, train_data, test_data=None, *, epoch: int, device="cpu", lr=0.001) -> ...:
        ...
        for e in range(epoch):
            ...
            if test_data is not None:
                auc, accuracy, rmse, recall, precision, f1 = self.eval(test_data, device=device)
    ...
    def eval(self, test_data, device="cpu") -> tuple:
        ...
        return roc_auc_score(y_true, y_pred), accuracy_score(y_true, np.array(y_pred) >= 0.5),\
            mean_squared_error(y_true, y_pred) ** 0.5, recall_score(y_true, np.array(y_pred) >= 0.3),\
            precision_score(y_true, np.array(y_pred) >= 0.3), f1_score(y_true, np.array(y_pred) >= 0.3)
