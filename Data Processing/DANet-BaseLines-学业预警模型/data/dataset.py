...
def student_behavior():
    target = 'label'
    data = pd.read_excel('./data/student_behavior/DataSet_V5_filled.xlsx').drop(['xh'], axis=1)
    # data = pd.read_excel('./data/student_behavior/DataSet_V5_smote.xlsx')
    train_idx = pd.read_csv('./data/student_behavior/train_idx.csv')['0'].values
    train = data.iloc[train_idx, :]
    valid_idx = pd.read_csv('./data/student_behavior/valid_idx.csv')['0'].values
    valid = data.iloc[valid_idx, :]
    test_idx = pd.read_csv('./data/student_behavior/test_idx.csv')['0'].values
    test = data.iloc[test_idx, :]
...
def get_data(datasetname):
    if datasetname == 'forest':
        ...
        return click()
    elif datasetname == 'student_behavior':
